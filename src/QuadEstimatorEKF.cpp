...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  rot_mat(2,2)=cos(theta)*cos(theta);

  V3F euler_int = rot_mat * gyro;

  float predictedPitch = pitchEst + dtIMU * euler_int.y;
  float predictedRoll = rollEst + dtIMU * euler_int.x;
  ekfState(6) = ekfState(6) + dtIMU * euler_int.z;
...
V3F intertial_accel = attitude.Rotate_BtoI(accel);

predictedState(3) = curState(3) + intertial_accel.x*dt;
predictedState(4) = curState(4) + intertial_accel.y*dt;
predictedState(5) = curState(5) + intertial_accel.z*dt;
